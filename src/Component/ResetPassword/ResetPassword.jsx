...
export default function ResetPassword() {
    ...
    async function resetPassword(values) {
        try {
            ...
            `http://spark1.runasp.net/api/Auth/Reset-password`,
            {
                email: values.email,
          ...
          // التحقق من وجود استجابة من السيرفر
            if (error.response) {
            ...
            // إذا فيه رسالة مباشرة
            } else if (data.message) {
                toast.error(data.message);
            } else {
                toast.error("Please check your email or code.");
            }
      
          // لو الخطأ من الشبكة أو ما فيه استجابة
          } else if (error.request) {
            toast.error("No response from the server. Please check your connection.");
      
          // لو حصل خطأ قبل ما يوصَل للطلب (مثلاً في الكود)
          } else {
        ...
        function validateForm(values) {
            ...
            if (!values.newPassword) {
                ...
                errors.newPassword = 'Invalid password. Enter password. It contains uppercase and lowercase letters, special characters($!%*?@&) only, and numbers.'
            }
